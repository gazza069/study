...
class AddressShow extends Component {
    ...
    getAddress(email){
        ...
        doAction(){
            ...
            let to = Lib.encodeEmail(this.props.email);
            let db = firebase.database();
            let d = new Date().getTime();
            let ref = db.ref('address/' + from + '/' + to + '/message' + d);
            ref.set('to: ' + this.state.input);
            let ref2 = db.ref('address/' + to + '/' + from + '/message/' +d);
            ref2.set('from: ' + this.state.input);
            let ref3 = db.ref('adress/' + to + '/' + from + '/check/');
            ref3.set(true);
            this.setState({input:''})
        }
